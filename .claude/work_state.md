...
최근 업데이트: 2026-05-31 (세션 41 — 프로덕션 게시물 영속성 보강 3건 배포)

## ✅ 2026-05-31 완료 (세션 41 — 게시물 영속성 보강)

휘발성 디스크(Render 무료)에서 공개 게시물 유실/초기화 방지 보강 3건 운영 반영.

| 커밋 | 내용 | 효과 |
|------|------|------|
| `61c99607` | 보강1: 게시물/관리자 쓰기 → 10분 디바운스 Drive 백업 | 프로덕션발 글이 재배포 전 Drive에 영속 |
| `61c99607` | 보강2: 복원 시 community_posts union 병합 | 백업 이후 라이브 글 덮어쓰기 방지 |
| `d435f5ad` | 안전가드: candidates>=100일 때만 Drive 업로드 | 빈/부분 DB가 좋은 백업 덮는 사고 차단 (fail-safe) |

- 검증: py_compile PASS / union 단위테스트 PASS / backup(dry_run) 346글·5185KB(프로덕션 일치) OK
- 백업: `20260531_092056__drive-safety-guard-deploy` (DB 해시 3ad84a49e983)
- 전부 추가형 변경 — 기존 기능 훼손 없음. 모든 변경이 백업을 더 보수적으로 만드는 방향.
- ⚠️ 무관 외부 커밋 `9a31442e`(doc_processor) mirror에 존재 — 본 세션 작업 아님(인지용)
