...
## [RULE-SECURITY-FIRST] 보안 우선 설계 원칙 — Haiku/Sonnet/Opus 모두 적용

> 이 규칙은 모든 모델, 모든 작업에 예외 없이 적용된다.
> 새 기능 설계 전 아래 체크리스트를 내부적으로 완료할 것.

### 크리덴셜 아키텍처 (단일 진실 공급원)

```
모든 크리덴셜의 흐름:
  pw.py GUI → bx (암호화) → 각 시스템 자동 동기화
                              ├── ClaudeBlog/secrets.enc
                              ├── Render 환경변수
                              └── 기타 프로그램
```
...
**신규 크리덴셜 추가 시 필수 3단계:**
1. `pw.py` PRESETS에 등록
2. `CLAUDEBLOG_SYNC_KEYS` 또는 `RENDER_SYNC_KEYS`에 추가
3. 해당 프로그램이 bx 경유로 읽도록 수정

**절대 금지:**
- 소스코드/설정파일에 크리덴셜 하드코딩
- bx 우회하여 파일에서 직접 읽기 (secrets.enc 제외 — ClaudeBlog 전용)
- 환경변수에 평문 저장

### 신규 기능 설계 체크리스트 (코드 작성 전 내부 확인)

```
[ ] 이 기능이 사용하는 크리덴셜은 무엇인가?
[ ] 크리덴셜이 바뀌면 어디가 깨지는가? → 깨지지 않도록 bx 경유로 설계
[ ] 외부 입력(사용자/API/파일)을 신뢰하지 않고 검증하는가?
[ ] 실패 시 기본값이 안전한가? (fail-closed)
[ ] 로그에 크리덴셜이 노출되지 않는가?
[ ] 인증 없이 접근 가능한 경로가 없는가?
[ ] 악성 HWP/PDF/파일 매크로 실행 경로가 차단되어 있는가?
```
...
| 경로 | 방어 |
|------|------|
| 문서 매크로 (HWP/Office) | 한글 ScriptRun=0 유지 / Office 매크로 비활성화 |
| 외부 .exe 실행 | AppData\Roaming 경로 실행 bridge_guard 차단 |
| 예약 태스크 등록 | schtasks /create bridge_guard 차단 |
| 크리덴셜 탈취 후 재사용 | pw.py 정기 교체 + 모든 연동 자동 갱신 |
| 난독화 스크립트 | PowerShell -EncodedCommand bridge_guard 차단 |

### 크리덴셜 교체 원칙

- **정기 교체 주기**: 보안 이벤트 발생 즉시 + 90일마다
- **교체 순서**: pw.py에서 1회 입력 → 모든 연동 자동 반영
- **교체 후 검증**: tg_notify.py 테스트 메시지 수신 확인

---

## [RULE-AUTONOMY] 자율 결정 원칙

Claude는 A/B/C 선택지가 생기면 스스로 최적안을 선택 후 진행한다.

- **선택 후 진행**: 선택지 나열 + 사용자 대기 금지 → 최적안 결정 + 한 줄 이유 + 즉시 실행
- **되돌릴 수 없는 작업은 확인 필수**:
  - DB hard-delete / 대량 마이그레이션 (>100건 변경)
  - Render 배포 / git force-push / branch 삭제
  - 외부 시스템 변경 (메일 발송 / Slack / 결제)
  - 네트워크 설정 변경 (CLAUDE.md [LOCK] 네트워크 참조)
- **자율 처리 대상**:
  - 코드 수정 / 테스트 실행 / git commit / 파일 생성
  - 기술 선택 (A/B/C 옵션 중 하나)
  - 에러 디버깅 / 롤백 / 재시도
  - 보안 영향 없는 리팩토링
- **로그 형식**: `[자율 선택] A안 채택 (이유: 1줄)` — 완료 보고에 포함

예외: 사용자가 "확인 후 진행" / "승인 기다려" 명시하면 원칙 무시하고 대기.

---

## [RULE-MODEL] 매 응답 모델 추천 출력

날짜시간 줄 바로 아래에 한 줄 출력:
`🤖 추천: [Opus/Sonnet/Haiku] — [이유 한 줄]`

판단 기준:
- Opus: 아키텍처 설계 / 보안 감사 / DB 스키마 변경 / 복잡한 리팩토링
- Sonnet: 단순 코드 수정 / 블로그 작성 / RPA 스크립트 / 일반 구현
- Haiku: 빠른 조회 / 단순 검색 / 파일 확인 / 간단한 질문
...
### [RULE-PROMPT] 클코 명령어 전달 규칙

1. **백업 라인 첫 줄 필수**: 모든 클코 명령어 프롬프트 첫 줄은 반드시 백업 명령어
2. **직렬/병렬 구분 명시**: 각 작업 앞에 `[직렬]` 또는 `[병렬]` 태그 표시
3. **코드블록 통합**: 프롬프트 전달 시 전체를 하나의 코드블록에 순서대로 묶어서 제공

예시 형식:
```
[백업] git add -A && git commit -m "pre: 작업명"

[병렬] 작업A - 설명
[병렬] 작업B - 설명
[직렬] 작업C - 작업A/B 완료 후 실행
```
