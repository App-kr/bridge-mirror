...
# ── 로그인 이벤트 ─────────────────────────────────────────────────────────────
_login_needed_event = threading.Event()   # RPA가 로그인 필요 시 set
_login_done_event   = threading.Event()   # 유저가 "로그인 완료" 누르면 set

def notify_login_needed():
    """크롤링 로그인 실패 시 RPA가 호출 — 오버레이에 로그인 배너 표시."""
    _login_needed_event.set()
    _login_done_event.clear()

def wait_for_login_done(timeout: float = 300) -> bool:
    """로그인 완료 버튼 클릭까지 대기. True=완료, False=타임아웃."""
    return _login_done_event.wait(timeout=timeout)

def reset_login_state():
    """로그인 이벤트 초기화 (다음 실행 준비)."""
    _login_needed_event.clear()
    _login_done_event.clear()
...
class RPAOverlay:
    ...
    def _build_working(self):
        ...
        # ── 로그인 필요 배너 (평소엔 숨김, 쿠키 만료 시 자동 표시) ────────────
        _login_banner = tk.Frame(btn_outer, bg="#2c1000")
        _login_lbl = tk.Label(_login_banner,
                              text="크롬 창에서 로그인 후 아래 버튼을 누르세요",
                              font=tkfont.Font(family="Malgun Gothic", size=11),
                              bg="#2c1000", fg="#ff9500")
        _login_lbl.pack(pady=(6, 2))
        _login_done_btn = tk.Label(_login_banner, text="로그인 완료",
                                   font=tkfont.Font(family="Malgun Gothic", size=13, weight="bold"),
                                   bg="#2c1000", fg="#30d158", cursor="hand2")
        _login_done_btn.pack(fill="x", ipady=8, padx=12, pady=(0, 6))
        def _on_login_done():
            _login_done_event.set()
            _login_needed_event.clear()
            _login_banner.pack_forget()
            tk.Frame(btn_outer, bg=self.SEP, height=1).pack(fill="x", before=_login_banner)
        _login_done_btn.bind("<Button-1>", lambda e: _on_login_done())
        _login_done_btn.bind("<Enter>",  lambda e: _login_done_btn.configure(bg="#003810"))
        _login_done_btn.bind("<Leave>",  lambda e: _login_done_btn.configure(bg="#2c1000"))
        tk.Frame(btn_outer, bg=self.SEP, height=1).pack(fill="x")

        def _poll_login_needed():
            try:
                if not root.winfo_exists():
                    return
                if _login_needed_event.is_set() and not _login_banner.winfo_ismapped():
                    _login_banner.pack(fill="x")
                root.after(500, _poll_login_needed)
            except Exception:
                pass
        root.after(500, _poll_login_needed)
        ...
        # 계정설정: 파란 텍스트
        _ACCT_HOV = "#001020"
        acct_btn = tk.Label(main_row, text="계정설정",
                            font=tkfont.Font(family="Malgun Gothic", size=13),
                            bg=self.BG, fg="#0a84ff", cursor="hand2")
        acct_btn.pack(side="left", expand=True, fill="both", ipady=10)
        def _do_acct_setup():
            import threading as _t
            _t.Thread(target=ask_vault_setup, daemon=True).start()
        acct_btn.bind("<Button-1>", lambda e: _do_acct_setup())
        acct_btn.bind("<Enter>", lambda e: acct_btn.configure(bg=_ACCT_HOV))
        acct_btn.bind("<Leave>", lambda e: acct_btn.configure(bg=self.BG))
    ...
    def _build():
        ...
        if _ico.exists():
            try:
                ...
                pass
        root.title("Craig RPA")
        # overrideredirect 제거 — 태스크바에 표시되어 게임 중에도 접근 가능
        root.attributes("-topmost", True)
        ...
        # ── Windows 포커스 강화 (게임 위로 올라오도록) ──
        try:
            import ctypes as _ct3
            # AllowSetForegroundWindow → SetForegroundWindow 조합으로 강제 포커스
            _ct3.windll.user32.AllowSetForegroundWindow(-1)  # ASFW_ANY
            root.after(100, lambda: _ct3.windll.user32.SetForegroundWindow(root.winfo_id()))
            root.after(300, lambda: _ct3.windll.user32.SetForegroundWindow(root.winfo_id()))
        except Exception:
            pass
