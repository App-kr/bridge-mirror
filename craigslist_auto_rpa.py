    ...
    try:
        ...
        if "further verification" in src_now:
            ...
            for _retry in range(3):
                print(f"  ⏳  [{_retry+1}/3] 5분 대기 — {CL_EMAIL} 이메일에서 링크 클릭 후 자동 재시도")
                countdown(300, f"재시도 대기 ({_retry+1}/3)")
                try:
                    driver.get(CL_LOGIN_URL)
                    _delay(2, 3)
                    _el = WebDriverWait(driver, 15).until(
                        EC.presence_of_element_located((By.ID, "inputEmailHandle"))
                    )
                    _type_slow(_el, CL_EMAIL)
                    _delay()
                    _pw = driver.find_element(By.ID, "inputPassword")
                    _type_slow(_pw, CL_PASSWORD)
                    _delay(0.5, 1.0)
                    _pw.send_keys(Keys.RETURN)
                    try:
                        WebDriverWait(driver, 12).until(_is_logged_in)
                        print(f"  [LOGIN] 재시도 {_retry+1}회 성공 ✅")
                        return True
                    except Exception:
                        pass
                    src2 = driver.page_source.lower()
                    if "further verification" not in src2:
                        break
                except Exception as _re:
                    print(f"  [RETRY {_retry+1} ERROR] {_re}")
            return False
