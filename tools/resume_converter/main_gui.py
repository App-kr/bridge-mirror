...
class QueueItem:
    ...
    files_multi: dict = field(default_factory=dict)   # ftype → [Path, ...]
...
class BridgeConverterApp:
    ...
    def __init__(self):
        ...
        self._active_queue_idx: int = -1   # 현재 선택된 대기열 인덱스 (-1 = 미선택)
    ...
    def _build_left(self, parent) -> tk.Frame:
        ...
        # ── 대기열 ─────────────────────────────────────────────
        qf = tk.LabelFrame(panel, text="처리 대기열",
                            font=(F, 11, "bold"),
                            bg=C_SIDE, fg=C_TEXT, padx=6, pady=4,
                            relief="flat",
                            highlightbackground=C_BORDER, highlightthickness=1)
        qf.pack(fill="both", expand=True, padx=12, pady=4)
        ...
        self._queue_tv.tag_configure("selected_row", background="#D1E7DD")

        q_scroll = ttk.Scrollbar(qf, orient="vertical", command=self._queue_tv.yview)
        self._queue_tv.configure(yscrollcommand=q_scroll.set)
        self._queue_tv.pack(side="left", fill="both", expand=True)
        q_scroll.pack(side="right", fill="y")

        # 대기열 선택 시 → 해당 사람의 파일 목록으로 전환
        self._queue_tv.bind("<<TreeviewSelect>>", self._on_queue_select)
    ...
    def _file_context_menu(self, event):
        """우클릭 → 파일 타입 변경 메뉴."""
        iid = self._file_tv.identify_row(event.y)
        if not iid or iid in self._group_iids.values():
            return  # 그룹 헤더에는 메뉴 없음
        self._file_tv.selection_set(iid)
        ...
        for ft, label in type_map.items():
            if ft == old_ftype:
                continue
            menu.add_command(
                label=f"변경: {label}",
                command=lambda new_ft=ft: self._change_file_type(iid, old_ftype, new_ft, fname))
        menu.tk_popup(event.x_root, event.y_root)

    def _change_file_type(self, iid: str, old_ft: str, new_ft: str, fname: str):
        """파일 타입 변경 — Treeview + _files_multi + _files 모두 갱신."""
        # _files_multi에서 이동
        file_path = None
        if old_ft in self._files_multi:
            for i, p in enumerate(self._files_multi[old_ft]):
                if p.name == fname:
                    file_path = self._files_multi[old_ft].pop(i)
                    break
            if not self._files_multi[old_ft] and old_ft in self._files:
                del self._files[old_ft]
            elif self._files_multi[old_ft]:
                self._files[old_ft] = self._files_multi[old_ft][-1]

        if file_path is None:
            return

        if new_ft not in self._files_multi:
            self._files_multi[new_ft] = []
        self._files_multi[new_ft].append(file_path)
        self._files[new_ft] = file_path

        # Treeview: 기존 행 삭제 → 새 그룹에 추가
        vals = self._file_tv.item(iid, "values")
        self._file_tv.delete(iid)
        new_gid = self._group_iids.get(new_ft, self._group_iids.get("unknown"))
        if new_gid:
            self._file_tv.insert(new_gid, "end",
                                 text="", values=vals, tags=(new_ft,))
            self._file_tv.item(new_gid, open=True)
        # 대기열 슬롯 동기화
        self._save_current_to_queue()
        ...
        for item in self._queue:
            ...
            if not item.candidate_id:
                messagebox.showwarning("입력 필요",
                    f"슬롯 {item.queue_id}: 강사 번호를 입력하세요.")
                return
            if not item.files:
                messagebox.showwarning("파일 없음",
                    f"슬롯 {item.queue_id} ({item.candidate_id}): 파일이 없습니다.")
                return
    ...
    def _rebuild_file_tv_from_multi(self):
        """_files_multi 기준으로 Treeview 파일 목록 재구성."""
        # 기존 자식 모두 삭제
        for ft, gid in self._group_iids.items():
            for child in self._file_tv.get_children(gid):
        ...
        # 타입별 파일 재추가
        for ftype, paths in self._files_multi.items():
            gid = self._group_iids.get(ftype, self._group_iids.get("unknown"))
            if gid is None:
                continue
            for p in paths:
    ...
    def _select_queue_row(self, idx: int):
        """대기열 Treeview에서 특정 행을 프로그램적으로 선택."""
        rows = list(self._queue_tv.get_children(""))
        if 0 <= idx < len(rows):
            self._queue_tv.selection_set(rows[idx])
            self._queue_tv.see(rows[idx])
