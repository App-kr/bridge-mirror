...
_RPA_ALL_RE         = re.compile(r'(?:각|모든|전체|all)\s*(?:계정|account)', re.IGNORECASE)
_RPA_ALL_N_RE       = re.compile(r'(?:각|모든|전체)\s*계정\s*(\d+)\s*건', re.IGNORECASE)
...
# 맛족도 직접 키워드
_MATJOKDO_KEYS  = ("맛족도", "matjokdo", "블로그 작업해", "블로그작업해", "블로그 작업 해줘",
                   "블로그작업해줘", "댓글작업", "댓글 작업")
...
_TASK_TRIGGER_RE = re.compile(
    r'(?:해줘|해봐|해봐줘|해봐요|해주세요|실행해|실행해줘|작업해줘|작업해|처리해줘|처리해|'
    r'시작해줘|시작해|완료해줘|마무리해줘|마무리해|고쳐줘|고쳐|수정해줘|수정해|'
    r'올려줘|올려|돌려줘|돌려|확인해줘|확인해|체크해줘)',
    re.IGNORECASE
)
...
def _handle_api_key_update(chat_id: str, new_key: str):
    """sk-ant-api... 형식 키가 붙여넣어지면 bx vault에 자동 저장 후 봇 재시작."""
    try:
        send(chat_id, "🔑 Anthropic API 키 감지됨 — 저장 중...")
        from bx import _store as bx_store
        bx_store("ANTHROPIC_API_KEY", new_key)
        # 캐시 무효화 — 실패 캐시 제거
        for k in list(_key_cache.keys()):
            if "anthropic" in k:
                del _key_cache[k]
        send(chat_id, "✅ 키 저장 완료. 봇 재시작 중...")
        _log(f"[KEY] Anthropic API key updated via Telegram, len={len(new_key)}")
        # 자기 자신 재시작 (새 키로)
...
def _try_direct_cmd(chat_id, text: str) -> bool:
    ...
    # ── 맛족도 블로그 직접 실행 ──────────────────────────────────
    if any(k in tl for k in _MATJOKDO_KEYS):
        threading.Thread(target=_run_matjokdo, args=(chat_id,), daemon=True).start()
        return True
    ...
    # ── RPA 전체 계정 (각 계정 N건) ──────────────────────────────
    m_all_n = _RPA_ALL_N_RE.search(tl)
    if m_all_n:
        limit = max(1, min(int(m_all_n.group(1)), 20))
        send(chat_id, f"[RPA] 전체 계정 각 {limit}건 시작...")
        def _run_all():
            for acc in RPA_ACCOUNTS:
                send(chat_id, f"[RPA] {acc} 실행 중 ({limit}건)")
                _run_rpa_direct(chat_id, acc, limit)
                time.sleep(2)
            send(chat_id, f"[RPA] 전체 완료 ({len(RPA_ACCOUNTS)}개 계정 × {limit}건)")
        threading.Thread(target=_run_all, daemon=True).start()
        return True

    if _RPA_ALL_RE.search(tl):
        send(chat_id, f"[RPA] 전체 계정 각 5건 시작...")
        def _run_all5():
            for acc in RPA_ACCOUNTS:
                send(chat_id, f"[RPA] {acc} 실행 중 (5건)")
                _run_rpa_direct(chat_id, acc, 5)
                time.sleep(2)
            send(chat_id, f"[RPA] 전체 완료 ({len(RPA_ACCOUNTS)}개 계정 × 5건)")
        threading.Thread(target=_run_all5, daemon=True).start()
        return True
    ...
    # "~해줘", "~실행해", "~작업해" 등 패턴 → 터미널에서 직접 실행
    if len(t) >= 5 and _TASK_TRIGGER_RE.search(t):
        _auto_terminal_task(chat_id, t)
        return True

    return False


def _run_matjokdo(chat_id):
    """맛족도 main.py --run 직접 실행."""
    mat = Path(r"Q:\Claudework\matjokdo_safe\main.py")
    if not mat.exists():
        send(chat_id, "[맛족도] 스크립트 없음", markup=keyboard([btn("메인 메뉴", "menu")]))
        return
    send(chat_id, "[맛족도] 블로그 작업 시작...")
    try:
        proc = subprocess.run(
            [PYTHON, "-X", "utf8", "-u", str(mat), "--run"],
            capture_output=True, text=True, timeout=600,
            cwd=str(mat.parent), encoding="utf-8", errors="replace",
        )
        out = (proc.stdout + proc.stderr).strip()[-2000:] or "(출력 없음)"
        send(chat_id, f"[맛족도] 완료\n<code>{_safe_text(out, 'HTML')[:1800]}</code>",
             parse_mode="HTML", markup=keyboard([btn("메인 메뉴", "menu")]))
    except subprocess.TimeoutExpired:
        send(chat_id, "[맛족도] 타임아웃(10분)", markup=keyboard([btn("메인 메뉴", "menu")]))
    except Exception as e:
        send(chat_id, f"[맛족도] 오류: {e}", markup=keyboard([btn("메인 메뉴", "menu")]))


def _auto_terminal_task(chat_id, task_text: str):
    """일반 텍스트 작업 요청 → AgentTerminal 자동 실행 (T1 슬롯)."""
    # 이미 실행 중인 터미널이 있으면 주입
    running = _find_running_terminal(chat_id)
    if running:
        running.inject_message(task_text)
        send(chat_id, f"[T{running.tid}] 요청 전달됨.")
        return
    # 신규 터미널 시작
    tid = "1"
    send(chat_id, f"[터미널] 작업 시작: {task_text[:60]}...")
    _launch_terminal(chat_id, tid, task_text, provider=None, api_key=None,
