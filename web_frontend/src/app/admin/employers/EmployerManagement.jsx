  ...
  return(
    <div style={{marginBottom:0,padding:"20px 24px 22px",background:isHighlighted?"#fffde7":"#fff",borderRadius:0,boxShadow:"none",borderBottom:"none",boxShadow:"none",position:"relative",borderLeft:item.blacklist?"4px solid #dc2626":isNewBlink?"4px solid #fecaca":"4px solid #e5e5e5",outline:isHighlighted?"2px solid #fbbf24":"none",outlineOffset:0,animation:isNewBlink?"newBarBlink 1s ease-in-out infinite":undefined}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:6}}>
        {editHeader
          ?<>
            <input value={tmpJNumber} onChange={e=>setTmpJNumber(e.target.value)} style={{fontFamily:"'Consolas',monospace",fontSize:"1rem",fontWeight:800,width:80,padding:"3px 8px",border:"2px solid #2563eb",borderRadius:6,outline:"none",color:"#2563eb",textAlign:"center"}} autoFocus onKeyDown={e=>{if(e.key==="Enter"){onUpdate(item.jNumber,{jNumber:tmpJNumber,name:tmpName,rawText:item.rawText.replace(/Job\.\s*\S+/,`Job. ${tmpJNumber}`)});setEditHeader(false);}if(e.key==="Escape")setEditHeader(false);}}/>
            <span style={{fontSize:"1rem",color:"#555",fontWeight:400}}>{item.region} {item.city}</span>
            <input value={tmpName} onChange={e=>setTmpName(e.target.value)} style={{fontSize:"1rem",fontWeight:600,padding:"3px 8px",border:"2px solid #2563eb",borderRadius:6,outline:"none",minWidth:160}} onKeyDown={e=>{if(e.key==="Enter"){onUpdate(item.jNumber,{jNumber:tmpJNumber,name:tmpName,rawText:item.rawText.replace(/Job\.\s*\S+/,`Job. ${tmpJNumber}`)});setEditHeader(false);}if(e.key==="Escape")setEditHeader(false);}}/>
          ...
          :<>
            <span onDoubleClick={()=>{setTmpJNumber(item.jNumber);setTmpName(item.name);setEditHeader(true);}} title="더블클릭 편집" style={{fontFamily:"'Consolas',monospace",fontSize:"1.15rem",fontWeight:800,background:item.jNumber.startsWith("N")?"#dbeafe":"#eff6ff",color:item.jNumber.startsWith("N")?"#1d4ed8":"#2563eb",border:`1px solid ${item.jNumber.startsWith("N")?"#93c5fd":"#bfdbfe"}`,padding:"4px 16px",borderRadius:6,letterSpacing:"0.04em",cursor:"text"}}>{item.jNumber}</span>
            <span style={{fontSize:"1rem",color:"#555",fontWeight:400}}>{item.region} {item.city}</span>
            <span onDoubleClick={()=>{setTmpJNumber(item.jNumber);setTmpName(item.name);setEditHeader(true);}} title="더블클릭 편집" style={{fontSize:"1rem",color:"#111",fontWeight:400,cursor:"text"}}>{item.name}</span>
          </>
        }
        {tags.map((tag,ti)=>{
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",userSelect:resizing!==null?"none":"auto",fontSize:`${fontSize}px`}}>
      ...
      <div style={{background:"#f8f8f8",borderBottom:"1px solid #ddd",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:4,alignItems:"center",padding:"4px 8px",borderBottom:"1px solid #eee",flexWrap:"wrap"}}>
          ...
          <button onClick={()=>{
            const rows=checked.size>0?visRows.filter(d=>checked.has(d.jNumber)):visRows;
            if(!rows.length)return;
            const ws=XLSX.utils.json_to_sheet(rows.map((r,i)=>({"No.":r.jNumber,"지역":r.region,"도시":r.city,"업체명":r.name,"이메일":r.email,"연락처":r.phone,"연령대":translateAge(r.teachingAge),"급여":r.salary,"메모":r.memo||""})));
            const wb=XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb,ws,"구인자목록");
            XLSX.writeFile(wb,`BRIDGE_구인자_${new Date().toISOString().slice(0,10).replace(/-/g,"")}.xlsx`);
          }} style={{...TB,border:"1px solid #16a34a",color:"#16a34a",background:"#f0fdf4",padding:"3px 10px"}}>📥 엑셀{checked.size>0?` (${checked.size})`:""}</button>
          <div style={{width:1,height:20,background:"#ddd",margin:"0 2px"}}/>
                    ...
                    if(col.key==="jNumber")return(
                      <td key="jNumber" style={{padding:0,border:"1px solid #e8e8e8",background:cellBg}}>
                        <XCell value={item.jNumber} onChange={v=>onUpdate(item.jNumber,{jNumber:v,rawText:(item.rawText||"").replace(/Job\.\s*\S+/,`Job. ${v}`)})}
                          ...
                          style={{fontFamily:"'Consolas',monospace",fontWeight:cs.bold?"800":"800",color:cs.color||"#2563eb",fontSize:`${fontSize}px`,textAlign:cs.align||"left"}}/>
                      </td>
                    );
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:1}}>
                          {(item.emails||[item.email]).map((em,j)=>(
                            <div key={j} style={{display:"flex",alignItems:"center",gap:2}}>
                              ...
                              <button onClick={()=>onMailTo&&onMailTo(item)} style={{background:"none",border:"none",cursor:"pointer",fontSize:"0.7rem",padding:0,color:"#93c5fd"}}>✉</button>
                            </div>
                          ))}
                          <button onClick={()=>{const ne=[...(item.emails||[item.email]),""];onUpdate(item.jNumber,{emails:ne});}} style={{background:"none",border:"none",color:"#2563eb",fontSize:"0.65rem",cursor:"pointer",textAlign:"left",padding:0}}>+ 추가</button>
                        </div>
                      </td>
                    );
...
export default function EmployerManagement(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        if(res.ok){const list=await res.json();setData(Array.isArray(list)?list:INIT_DATA);}
  ...
  const delRows=useCallback((jns)=>{
    setData(p=>p.filter(d=>!jns.includes(d.jNumber)));setChecked(new Set());
    jns.forEach(jn=>fetch(`${API_BASE}/api/employers/${encodeURIComponent(jn)}`,{method:"DELETE"}).catch(()=>{}));
  },[API_BASE]);
  const addNew=useCallback(()=>{
    const id=nextId();
    const item={jNumber:id,region:"서울",city:"강남",name:"NEW 어학원",email:"new@test.com",emails:["new@test.com"],phone:"010-0000-0000",contact:"",teachingAge:"Elementary",salary:"2,500,000",status:"new",blacklist:false,active:true,isNew:true,confirmed:false,tags:[],memo:`(서울 강남 NEW어학원 신규접수)`,rawText:`Seoul Gangnam\nJob. ${id}\nStarting Date : September\nTeaching Age : Elementary`};
    setData(p=>[item,...p]);
    if(cRef.current)cRef.current.scrollTop=0;
    fetch(`${API_BASE}/api/employers`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(item)}).catch(()=>{});
  },[API_BASE]);
  const updateItem=useCallback((jn,u)=>{
    ...
    // DB 동기화 (fire & forget)
    fetch(`${API_BASE}/api/employers/${encodeURIComponent(jn)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)}).catch(()=>{});
  },[API_BASE]);
