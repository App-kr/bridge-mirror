...
/* ═══════════════════════════════════════════════════════
   BRIDGE Canvas Spreadsheet v2 — React Wrapper
   - 전체 데이터 자동 로드 (3000+건)
...
/* ── DB → DataRow mapping (원본 BridgeAdminSheet 동일 로직) ── */
...
export default function BridgeCanvasSheet() {
  ...
  // Periodic check for new web_form submissions
  useEffect(() => {
  ...
  /* ── Stable GridEngine callbacks (never change — use refs) ── */
  ...
  /* ── Column/Row uniform sizing ── */
  const setAllColWidth = useCallback((w: number) => {
    setCols(prev => prev.map(c => ({ ...c, w })))
    setCtx(null)
  }, [])
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', flex: '0 0 220px' }}>
          ...
          />
          <span style={{ position: 'absolute', left: 7, top: '50%', transform: 'translateY(-50%)', color: '#666', fontSize: 13 }}>🔍</span>
        </div>
      ...
      {ctx && (
        ...
        >
          ...
          <div style={{ height: 1, background: '#e5e7eb', margin: '3px 0' }} />
          <CtxItem label="📏 모든 열 너비 동일 (80px)" onClick={() => setAllColWidth(80)} />
          <CtxItem label="📏 모든 열 너비 동일 (120px)" onClick={() => setAllColWidth(120)} />
        </div>
      )}
