...
export default function AdminAuthModal({ onLogin, onClose, waking = false }) {
        ...
        <div className="p-6">
            ...
            <div className="space-y-4">
              ...
              {waking && (
                <p className="text-amber-500 text-sm text-center animate-pulse">
                  서버를 깨우는 중입니다. 잠시만 기다려주세요...
                </p>
              )}
              <button
                onClick={handlePasswordSubmit}
                disabled={loading}
                className="w-full bg-gray-950 hover:bg-gray-800 active:scale-95 text-white font-bold py-3 rounded-xl transition-all text-base disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {loading ? (waking ? '서버 깨우는 중...' : '인증 중...') : '접속'}
              </button>
            </div>
          )}
          ...
          {step === 'captcha' && captchaType === 'puzzle' && puzzleSet && (
            <div className="space-y-4">
              ...
              <div
                className="mx-auto rounded-xl overflow-hidden border-2 border-gray-200"
                style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, width: 'fit-content' }}
              >
                {[0, 1, 2].map(q => (
